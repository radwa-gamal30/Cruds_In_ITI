...
<body>
  ...
  <table class="table table-hover " style="width:80vw; margin-left: 100px;padding:30px 50px;">
        ...
        <tr>
            ...
            <td>
              {{-- @dd('storage/' . $student->image); --}}
              <img src="{{ asset('storage/' . $student->image) }}" alt="Student Image" style="width: 30%; height:auto;">
            </td>
            <td>
         
              {{-- @dd($student->track->id) --}}
              {{ $student->track ? $student->track->name : 'No Track Assigned' }}
              
             </td>
            <td class="col">
              <a href="{{route('students.show',$student->id)}}"><x-button color="success" inside="view"></x-button></a>
              <form action="{{ route('students.destroy_student', $student->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
